...
var targetMaxDeltaAngle : float = 10;
...
function Update (){
	...
	if(firing){
		 GameObject.Find(targetRootName[0]).transform.GetComponent("Health").health -= 1;
	}
}
...
function sentryAI(){
	...
	//Target.
	if(foundTarget){
		SendMessage("Shooting", SendMessageOptions.DontRequireReceiver);
		firing = true;
...
function GetTargets() : GameObject[]{
	...
	for (var i = 0; i < targetRootName.Length; i++){
		if(GameObject.Find(targetRootName[i]) != null){
			if(Vector3.Distance(GameObject.Find(targetRootName[i]).transform.position, transform.position) <= 50.0F){
